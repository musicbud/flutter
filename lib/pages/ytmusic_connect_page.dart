...
class YtMusicConnectPage extends StatefulWidget {
  ...
  const YtMusicConnectPage({Key? key, required this.apiService}) : super(key: key);
...
class _YtMusicConnectPageState extends State<YtMusicConnectPage> {
  ...
  void _handleToken(String token) async {
    ...
    try {
      final result = await widget.apiService.handleYtMusicToken(token);
      setState(() {
      ...
      setState(() {
        _errorMessage = 'Error handling YouTube Music token: $e';
      });
    } finally {
      setState(() {
        _isLoading = false;
  ...
  Future<void> _connect() async {
    ...
    try {
      final url = await widget.apiService.connectService('ytmusic');
      if (kIsWeb) {
        html.window.open(url, 'YouTube Music Auth', 'width=800,height=600');
      } else {
        // Handle mobile platforms here
      }
    } catch (e) {
      setState(() {
        _errorMessage = 'Error connecting to YouTube Music: $e';
      });
    } finally {
      setState(() {
        _isLoading = false;
  ...
  Future<void> _updateLikes() async {
    ...
    try {
      final result = await widget.apiService.updateLikes('ytmusic');
      if (result['success']) {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            if (_isConnected)
              Text('YouTube Music connected for user: $_displayName ($_userId)'),
            SizedBox(height: 20),
            ElevatedButton(
              onPressed: _isLoading ? null : _connect,
              child: _isLoading ? CircularProgressIndicator() : Text('Connect YouTube Music'),
            ),
            SizedBox(height: 20),
            ElevatedButton(
